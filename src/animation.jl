...
Creates an animated gif of the given stimuli (can animate more than one at a time for
side-by-side comparison). Super hack-y: Uses `PyPlot`'s `imshow` function for each frame of
the stimulus, writes them to separate `.gif` files, then uses a system call to `convert` or
the `FileIO` package to create the animated gif. Will delete the individual frames after
it's done, and will do its best to be graceful about this (i.e. if it's interrupted, that
sort of thing).

Note that it will remove any filename extensions from `filename` using `remove_extension`
and then append ".gif". In particular, if there are any `'.'` characters in `filename`, make
sure it ends with `.gif`

Returns `success, status` where `success` is a boolean indicated success or failure of the
process to produce an animated gif, and `status` is an array of strings and `Exception`s
representing status messages produced throughout (e.g. warnings about which frames got
skipped, that sort of thing).
...
function animated_gif(stimulus...; filename="default.gif", verbose=0,
    ...
    try
        ...
        if method == "ImageMagick"
            if verbose > 0
                println("animaged_gif: loading files for ImageMagick")
                print("animated_gif: [")
            end
            frame1 = load(joinpath(temp_dir, "$fname_root-0001$intermediate_extension"))
            ...
            if verbose > 0
                println("]")
                print("animated_gif: saving gif...")
                save(joinpath(floc,gif_filename), A, fps=fps)
                println("done")
                println("Created file $(joinpath(floc,gif_filename))")
            end
            success = true
        else
            # default: do a system call to use convert
            if verbose > 0
